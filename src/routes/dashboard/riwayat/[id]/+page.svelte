			...
			{#each sections as section, i (section.id)}
				<div class="section-block group/section relative px-12 {i === 0 ? 'pt-8' : ''} {i === sections.length - 1 ? 'pb-8' : ''}">
					...
					{:else}
						...
						<button
							onclick={() => startEdit(i)}
							class="no-print edit-section-btn absolute right-2 top-2 flex items-center gap-1 rounded-md border border-yellow-300 bg-yellow-50 px-2 py-1 text-xs text-yellow-700 shadow-sm transition-all hover:bg-yellow-100 hover:border-yellow-400 hover:text-yellow-900 opacity-0 group-hover/section:opacity-100"

<script>
	...
	function saveEdit() {
		...
		lkpdData.output = markdownSections.join('\n');
		const fullHtml = renderMarkdownWithImages(lkpdData.output, lkpdData.images);
	...
	onMount(() => {
		...
		try {
			...
			markdownSections = splitMarkdownBySections(lkpdData.output);
